...
void borraPantalla(void) {M5.Lcd.fillScreen(COLOR_FONDO);}
...
void pintaTitulo(void)
  ...
  if(getValorPrincipal()!=REPOSO)
    {     
    M5.Lcd.fillRect(ANCHO_CERO, ALTO_CERO, ANCHO_TOTAL, ALTO_TITULO,COLOR_TITULO);
    M5.Lcd.setTextDatum(TC_DATUM);// Set text datum to top centre
    M5.Lcd.setTextColor(COLOR_LETRAS_TITULO, COLOR_TITULO);// Set text colour to orange with black background
...
*/
  ...
  if(sateliteRegistrado(satelite))
    ...
    //Pinta el fondo
    M5.Lcd.fillRect(MARGEN_IZQUIERDO, ALTO_PRINCIPAL+ALTO_TITULO+2*SEPARADOR_VERTICAL, ANCHO_TOTAL-2*MARGEN_IZQUIERDO, ALTO_SECUNDARIO, TFT_NAVY);
    //pint el marco
...
void pintaLayout3(String valor, int satelite, String alarma="")//Modo reposo
  ...
  */
  ...
  y0=ALTO_TITULO+SEPARADOR_VERTICAL+2*ALTO_PRINCIPAL;
  ancho=ANCHO_TOTAL;
  alto=ALTO_PRINCIPAL;  
  M5.Lcd.setTextColor(TFT_WHITE, TFT_BLACK);// Set text colour to white with black background
  M5.Lcd.setTextDatum(ML_DATUM);// Set text datum to middle centre
  M5.Lcd.setFreeFont(FF18);                 // Select the font
  //M5.Lcd.drawString(getHora(), ANCHO_TOTAL/4, y0+alto/2, GFXFF);// Print the string name of the font  
  M5.Lcd.drawString(getHora(), x0, y0+alto/2, GFXFF);// Print the string name of the font  
  ...
  M5.Lcd.setTextDatum(MC_DATUM);// Set text datum to middle centre
  M5.Lcd.setFreeFont(FF18);                 // Select the font
  M5.Lcd.drawString((String(getConsigna(),1)+"C").c_str(), 3.5*ANCHO_TOTAL/4, y0+alto/2, GFXFF);// Print the string name of the font
