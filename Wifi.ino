...
/*  WiFiManagerParameter Nombre_Parametro("1","dispositivo",nombre_dispositivo.c_str(),MAX_LONG_NOMBRE_DISPOSITIVO+1,"Nombre del dispositivo");
  ...
  if (!wifiManager.startConfigPortal(("AP_"+nombre_dispositivo).c_str())) Serial.println("failed to connect and hit timeout");
  else 
    {
...
void WifiWD(void) {
  ...
  if (fallos>LIMITE_FALLOS_CONEXION) 
    {
    enviarMQTT("freeheap", "{\"IP\": \"" + String(getIP(debugGlobal)) + "\" , \"Uptime\": 1971, \"freeHeap\": " + String(ESP.getFreeHeap()) + ", \"potencia\": \"" + String(WiFi.RSSI()) + "\", \"lastResetReason0\": " + rtc_get_reset_reason(0) + ", \"lastResetReason1\": " + rtc_get_reset_reason(1) + "}");  
    ESP.restart();
    }
  }
